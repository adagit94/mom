import { validateSquare } from "./helpers.js";
...
export default class Matrix<T = unknown> {
  ...
  public setValue = (value: T, row: number, column: number): void =>
    setValue<T>(value, row, column, this.columns, this.matrix);
  ...
  public scale = (
    ...
    const { mat, rows, columns } = scale<T>(
      this.rows,
      rowsFactor,
      this.columns,
      columnsFactor,
  ...
  public clip = (origin: { row: number; column: number }, destination: { row: number; column: number }) => {
    const { mat, rows, columns } = clip<T>(origin, destination, this.columns, this.matrix);
