...
import { Sorter, getSorter, handleDiagonal, getOffset, recomputeProportionaly } from "./utils.js";
...
export const swapValues = <T = unknown>(
  rowA: number,
  columnA: number,
  rowB: number,
  columnB: number,
...
export const getValue = <T = unknown>(row: number, column: number, columns: number, mat: T[]): T =>
  mat[column + columns * row];
...
export const replaceDiagonal = <T = unknown>(
  newValues: T[],
  size: number,
  mat: T[],
  direction: "\\" | "/" = "\\"
...
export const mergeRows = <T = unknown>(
  index: number,
  mergeHandler: (a: T, b: T) => T,
  columns: number,
  mat: T[]
): T[] => {
...
export const mergeColumns = <T = unknown>(
  index: number,
  mergeHandler: (a: T, b: T) => T,
  columns: number,
  mat: T[]
...
export const splitRow = <T = unknown>(
  index: number,
  splitHandler: (a: T) => [T, T],
  columns: number,
  mat: T[]
): T[] => {
...
export const splitColumn = <T = unknown>(
  index: number,
  splitHandler: (a: T) => [T, T],
  columns: number,
  mat: T[]
...
export const sortRow = <T = unknown>(
  index: number,
  sorter: "asc" | "desc" | Sorter<T>,
  columns: number,
  mat: T[]
): T[] => {
  if (typeof sorter === "string") sorter = getSorter<T>(sorter);
...
export const sortColumn = <T = unknown>(
  index: number,
  sorter: "asc" | "desc" | Sorter<T>,
  columns: number,
  mat: T[]
...
export const scale = <T>(
  ...
  switch (valuesTransfer) {
    ...
    case ValuesTransfer.Proportional: {
      ...
      for (const slot of prevSlots) {
        const { row, column, value } = slot;
        const { row: newRow, column: newColumn } = recomputeProportionaly(
...
export const clip = <T>(
  ...
  columns: number,
  mat: T[]
): { mat: T[]; rows: number; columns: number } => {
  const offset = getOffset(origin, destination);

  if (offset.rows === 0 && offset.columns === 0) {
    return { mat: [mat[origin.column + columns * origin.row]], rows: 1, columns: 1 };
  }

  let rows: T[][] = [];

  if (offset.rows > 0) {
    const destinationRow = origin.row + offset.rows;

    for (let r = origin.row; r <= destinationRow; r++) {
      rows.push(getRow(r, columns, mat));
    }
  } else if (offset.rows < 0) {
    const destinationRow = origin.row - offset.rows;

    for (let r = origin.row; r >= destinationRow; r--) {
      rows.unshift(getRow(r, columns, mat));
    }
  } else {
    rows.push(getRow(origin.row, columns, mat));
  }

  if (offset.columns > 0) {
    const destinationColumn = origin.column + offset.columns;

    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];

      rows[i] = row.slice(origin.column, destinationColumn + 1);
    }
  } else if (offset.columns < 0) {
    const firstColumn = origin.column - offset.columns;

    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];

      rows[i] = row.slice(firstColumn, origin.column + 1);
    }
  } else {
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];

      rows[i] = [row[origin.column]];
    }
  }

  return {
    mat: rows.flat(),
    rows: Math.abs(offset.rows) + 1,
    columns: Math.abs(offset.columns) + 1,
  };
};
