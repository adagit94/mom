...
import { SlotHandler, create } from '../patterns.js';
...
export default class Matrix<T = unknown> {
  ...
  constructor(rows: number, columns: number, input: T[] | SlotHandler<T>) {
    this.rows = rows;
  ...
  public getRow = (index: number) =>
    structuredClone(getRow<T>(index, this.columns, this.matrix));
  public getColumn = (index: number) =>
    structuredClone(getColumn<T>(index, this.columns, this.matrix));
  public getDiagonal = (direction: '\\' | '/' = '\\') =>
    structuredClone(
      validateSquare(this.rows, this.columns) &&
        getDiagonal<T>(this.rows, this.matrix, direction)
    );
  ...
  public reverseDiagonal = (direction: '\\' | '/' = '\\') => {
    this.matrix =
      validateSquare(this.rows, this.columns) &&
      reverseDiagonal<T>(this.rows, this.matrix, direction);
  };
  ...
  public swapDiagonals = () => {
    this.matrix =
      validateSquare(this.rows, this.columns) &&
      swapDiagonals<T>(this.rows, this.matrix);
  };
  ...
  public replaceDiagonal = (newValues: T[], direction: '\\' | '/' = '\\') => {
    this.matrix =
      validateSquare(this.rows, this.columns) &&
      replaceDiagonal<T>(newValues, this.rows, this.matrix, direction);
  };
